...
def AStarSearch(maze, cost, start, end):
    ...
    num_rows, num_columns = np.shape(maze)       # get the dimensions of the maze
        ...
        for new_position in move:                # try each of the 4 possible moves:
            ...
            for child in children:               # loop through each child in children list (looking for best next step)
                ...
                child.h = (((child.position[0] - end_node.position[0]) ** 2) +
                           ((child.position[1] - end_node.position[1]) ** 2))  # h used as a "distance to end" metric
...
def drawGrid():
    ...
    for row in grid:
        for col in row:
            if col == 0:
                pygame.draw.rect(win, BLACK, (x_pos, y_pos, block_size, block_size), 3)  # white box (available path)
            elif col == 1:
